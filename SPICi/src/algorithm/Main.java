...
public class Main {
	public static void main(String[] args) throws UnsupportedEncodingException, FileNotFoundException, IOException {
		...
		Integer highest = Math.max(edges.stream().max((a, b) -> a[0] > b[0] ? 1 : -1).get()[0],
				edges.stream().max((a, b) -> a[1] > b[1] ? 1 : -1).get()[1]);

		int nVertices = highest;
		int nEdges = edges.size();

		// Choose initial capacity high enough to minimize frequency of rehashing
		HashMap<Integer, Double> degreeQ = new HashMap<Integer, Double>((int)Math.ceil(nVertices / 0.75) );
		...
		while (!degreeQ.isEmpty()) {
			...
			double seed1Value = degreeQ.get(seed1);
			System.out.println("Seed-One   " + seed1);
			
			//Find vertices adjacent to seed1
			...
			if (seed2 == -1) {
				...
				do {
					for (int j : clusterS.keySet()) {
						for (int i = 0; i < nEdges; i++) {
							Integer[] edge = edges.get(i);
							if ((edge[0] == j) && !tried.containsKey(edge[1]) && !clusterS.containsKey(edge[1])
									&& degreeQ.containsKey(edge[1])) {
								canidateQ.put(edge[1], Calculators.support(edge[1], clusterS, weights, edges, nEdges));
							}
							if ((edge[1] == j) && !tried.containsKey(edge[0]) && !clusterS.containsKey(edge[0])
									&& degreeQ.containsKey(edge[0])) {
					...
					if (!canidateQ.isEmpty()) {
						...
						double maxValue = canidateQ.get(maxSup);

						// System.out.println("Highest support = " + maxSup);
						// System.out.println("Support = " + maxValue;

						HashMap<Integer, Double> temp = new HashMap<Integer, Double>();
						temp.putAll(clusterS);
						temp.put(maxSup, maxValue);

						if ((maxValue >= (sThreshold * clusterS.size()
								* Calculators.density(clusterS, weights, edges, nEdges))
								&& Calculators.density(temp, weights, edges, nEdges) > dThreshold)) {
							clusterS.put(maxSup, maxValue);
						}
						temp.clear();
